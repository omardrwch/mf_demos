...
#-------------------------------------------------------------------------------
# Compare theoretical and computed spectra
#-------------------------------------------------------------------------------
...
hmin = c1_theo - np.sqrt(-2*c2_theo)
hmax = c1_theo + np.sqrt(-2*c2_theo)
hvec = np.linspace(hmin, hmax, 100)

Dh_theo = 1 + 0.5*np.power(hvec - c1_theo,2)/c2_theo

plt.figure()
plt.plot(hvec, Dh_theo, 'k-' ,label='theoretical spectrum')
plt.plot(mfa.spectrum.hq, mfa.spectrum.Dq, 'bo-' ,label='estimated spectrum')
plt.ylabel('D(h)')
plt.xlabel('h')
